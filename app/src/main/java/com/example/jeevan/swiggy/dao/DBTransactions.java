...
public class DBTransactions {
    ...
    private Order getOrderFromCursor(Cursor cursor) {
        ...
        order.setOrderId(cursor.getLong(cursor.getColumnIndex(OrderTable.KEY_ID)));
    ...
    public List<Restaurant> getAllRestaurants() {
        ...
        Cursor cursor = db.query(RestaurantsTable.TABLE_NAME, null, null, null, null, null, null);
    ...
    public Order getPreviousOrder(long userId, String occasion) {
        Order prevOrder = null;
        String where = OrderTable.KEY_ID + " = MAX(" + OrderTable.KEY_ID + ") AND " + OrderTable.KEY_USER_ID + " = ? AND " + OrderTable.KEY_OCCASION + " = ?";
        String[] args = {String.valueOf(userId), occasion};
        Cursor cursor = db.query(OrderTable.TABLE_NAME, null, where, args, null, null, null);
        if (cursor.moveToNext()) {
            prevOrder = getOrderFromCursor(cursor);
        }
        if (cursor != null) cursor.close();
        return prevOrder;
    }
    ...
    public Order getTrendingOrder(String occasion) {
        Order ret = null;
        String where = OrderTable.KEY_OCCASION + " = ?";
        String[] args = {occasion};
        String order = OrderTable.KEY_ID + " DESC";
        Cursor cursor = db.query(OrderTable.TABLE_NAME, null, where, args, null, null, order);
        if (cursor.moveToNext()) {
            ret = getOrderFromCursor(cursor);
        }
        if (cursor != null) cursor.close();
        return ret;
    }
